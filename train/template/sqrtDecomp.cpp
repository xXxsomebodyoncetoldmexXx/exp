...
vector<ll> process(vector<ll> vec){
  ll n = vec.size();
  ll len = (ll)(sqrt(n)) + 1;
  vector<ll> ret(len, 1  );

  for(ll i = 0; i < n; i++)
    ret[i/len] = f(ret[i/len], vec[i]);

  return ret;
}

ll query(vector<ll> v, vector<ll> ori, ll L, ll R){
  ll n = v.size();
  ll ans = 1;
  for(ll i = L; i <= R;){
    if(i%n == 0 && i+n-1 <= R){
...
int main(int argc, char const *argv[]) {
  ...
  for(auto &i : v)
    cin >> i;
  vector<ll> sqrtDecomp = process(v);
  for(auto i : sqrtDecomp)
    cout << i << sp;
  cout << el;
  cout << query(sqrtDecomp, v, 0, 9); 
