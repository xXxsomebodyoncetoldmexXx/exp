...
inline void update(vector<ll> &cur, ll node, ll value){
  for(ll i = node; i < cur.size(); i += i&(-i)){
    cur[i] = f(cur[i], value);
  }
}

inline ll get(vector<ll> &cur, ll node){
  ll ans = 0;
...
vector<ll> init(vector<ll> &v){
  vector<ll> ret(v.size()+1, 0);
...
int main(int argc, char const *argv[]) {
  ...
  vector<ll> test = {1,2,3,4,5,6,7};
  vector<ll> fw = init(test);
  for(auto i : fw)
    cout << i << sp;
  cout << el;
  cout << get(fw, 7);
